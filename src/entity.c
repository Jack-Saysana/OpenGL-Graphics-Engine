...
ENTITY *init_entity(MODEL *model) {
  ...
  if (model->num_colliders > 0) {
    ...
    for (size_t i = 0; i < model->num_colliders; i++) {
      ...
      vec6_zero(ent->np_data[i].s_hat[0]);
      vec6_zero(ent->np_data[i].s_hat[1]);
      vec6_zero(ent->np_data[i].s_hat[2]);
      vec6_zero(ent->np_data[i].Z_hat);
      vec6_zero(ent->np_data[i].Z_hat_A);
      vec6_zero(ent->np_data[i].coriolis_vector[0]);
      vec6_zero(ent->np_data[i].coriolis_vector[1]);
      vec6_zero(ent->np_data[i].coriolis_vector[2]);
      vec6_zero(ent->np_data[i].a_hat);
      vec6_zero(ent->np_data[i].v_hat);
      vec6_zero(ent->np_data[i].s_inner_I[0]);
      vec6_zero(ent->np_data[i].s_inner_I[1]);
      vec6_zero(ent->np_data[i].s_inner_I[2]);
      glm_vec3_zero(ent->np_data[i].accel_angles);
      glm_vec3_zero(ent->np_data[i].vel_angles);
      glm_vec3_zero(ent->np_data[i].joint_angles);
      glm_vec3_zero(ent->np_data[i].dofs[0]);
      glm_vec3_zero(ent->np_data[i].dofs[1]);
      glm_vec3_zero(ent->np_data[i].dofs[2]);
      glm_vec3_zero(ent->np_data[i].from_parent_lin);
      glm_vec3_zero(ent->np_data[i].joint_to_com);
      glm_vec3_zero(ent->np_data[i].s_inner_I_dot_s);
      glm_vec3_zero(ent->np_data[i].SZI);
      ent->np_data[i].inv_mass = 0.0;
      ent->np_data[i].Q = 0.0;
      ent->np_data[i].num_dofs = 1;
    }
  } else {
